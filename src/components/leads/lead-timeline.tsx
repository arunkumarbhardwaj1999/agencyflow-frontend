...
> = {
  lead_created: { label: "Lead created", icon: PlusCircle, color: "text-indigo-600 bg-indigo-50" },
  ...
  whatsapp_sent: { label: "WhatsApp sent", icon: MessageCircle, color: "text-emerald-600 bg-emerald-50" },
  attachment_uploaded: { label: "Attachment", icon: Paperclip, color: "text-slate-600 bg-slate-100" },
  converted_to_client: { label: "Converted", icon: CheckCircle2, color: "text-emerald-700 bg-emerald-50" },
  assigned: { label: "Assigned", icon: UserRound, color: "text-indigo-600 bg-indigo-50" },
  followup_scheduled: { label: "Follow-up", icon: CalendarClock, color: "text-amber-700 bg-amber-50" },
...
export function LeadTimeline({ events }: { events: LeadTimelineEvent[] }) {
  const grouped = useMemo(() => {
    const map = new Map<string, LeadTimelineEvent[]>();
  ...
  if (events.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-10 text-center text-sm text-slate-500">
        No activity yet. Notes, emails, stage changes, and uploads will appear here automatically.
      </div>
    );
  }
