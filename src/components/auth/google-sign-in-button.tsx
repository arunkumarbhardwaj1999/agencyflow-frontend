"use client";

import { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import { useMutation } from "@tanstack/react-query";
import { CredentialResponse, GoogleLogin } from "@react-oauth/google";
import { apiFetch } from "@/lib/api";
import { setTokens } from "@/lib/auth";
import type { GoogleRegisterPending, TokenResponse } from "@/lib/types";

const CLIENT_ID = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID ?? "";

type GoogleSignInButtonProps = {
  intent?: "login" | "register" | "invite";
  inviteToken?: string;
  onRegisterComplete?: (data: GoogleRegisterPending) => void;
};

function GoogleIcon() {
  return (
    <svg className="h-5 w-5 shrink-0" viewBox="0 0 24 24" aria-hidden>
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  );
}

export function GoogleSignInButton({
  intent = "login",
  inviteToken,
  onRegisterComplete,
}: GoogleSignInButtonProps) {
  const router = useRouter();
  const containerRef = useRef<HTMLDivElement>(null);
  const [buttonWidth, setButtonWidth] = useState(400);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
    const update = () => setButtonWidth(Math.max(Math.floor(el.offsetWidth), 280));
    update();
    const ro = new ResizeObserver(update);
    ro.observe(el);
    return () => ro.disconnect();
  }, []);

  const mutation = useMutation({
    mutationFn: async (credential: string): Promise<TokenResponse | GoogleRegisterPending> => {
      if (intent === "register") {
        return apiFetch<GoogleRegisterPending>("/auth/google/register", {
          method: "POST",
          body: JSON.stringify({ credential }),
        });
      }
      if (intent === "invite" && inviteToken) {
        return apiFetch<TokenResponse>("/auth/invite/accept-google", {
          method: "POST",
          body: JSON.stringify({ credential, token: inviteToken }),
        });
      }
      return apiFetch<TokenResponse>("/auth/google", {
        method: "POST",
        body: JSON.stringify({ credential }),
      });
    },
    onSuccess: (data) => {
      if ("registration_id" in data) {
        onRegisterComplete?.(data);
        return;
      }
      setTokens(data.access_token, data.refresh_token);
      router.push(intent === "invite" ? "/join/accepted" : "/dashboard");
    },
    onError: (err: Error) => setError(err.message),
  });

  if (!CLIENT_ID) {
    return (
      <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
        Google sign-in needs <code className="font-mono">NEXT_PUBLIC_GOOGLE_CLIENT_ID</code>
      </p>
    );
  }

  function onSuccess(res: CredentialResponse) {
    if (!res.credential) {
      setError("Google did not return a sign-in token");
      return;
    }
    setError(null);
    mutation.mutate(res.credential);
  }

  return (
    <div className="auth-google-block">
      {intent === "login" && (
        <div ref={containerRef} className={`auth-google-wrap ${mutation.isPending ? "pointer-events-none opacity-60" : ""}`}>
          <button type="button" className="auth-btn-google" tabIndex={-1} aria-hidden>
            <GoogleIcon />
            Continue with Google
          </button>
          <div className="auth-google-overlay" aria-label="Continue with Google">
            <GoogleLogin
              onSuccess={onSuccess}
              onError={() => setError("Google sign-in was cancelled or failed")}
              theme="outline"
              size="large"
              width={buttonWidth}
              text="continue_with"
              shape="pill"
            />
          </div>
        </div>
      )}

      {intent !== "login" && (
        <div ref={containerRef} className={`auth-google-wrap ${mutation.isPending ? "pointer-events-none opacity-60" : ""}`}>
          <button type="button" className="auth-btn-google" tabIndex={-1} aria-hidden>
            <GoogleIcon />
            {intent === "invite" ? "Sign up & accept" : "Continue with Google"}
          </button>
          <div className="auth-google-overlay" aria-label="Continue with Google">
            <GoogleLogin
              onSuccess={onSuccess}
              onError={() => setError("Google sign-in was cancelled or failed")}
              theme="outline"
              size="large"
              width={buttonWidth}
              text="continue_with"
              shape="pill"
            />
          </div>
        </div>
      )}

      {(error || mutation.isError) && (
        <p className="mt-2 text-sm text-red-600">{error ?? (mutation.error as Error).message}</p>
      )}
    </div>
  );
}
