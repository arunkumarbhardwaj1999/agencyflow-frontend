...
const socials = ["Fb", "Tw", "Ig", "Li"];

type LandingNavProps = {
  variant?: "light" | "dark" | "green";
};

export function LandingNav({ variant = "green" }: LandingNavProps) {
  const [open, setOpen] = useState(false);

  const onGradient = variant === "green";
      ...
      <header className="absolute inset-x-0 top-0 z-50 flex items-center justify-between px-6 py-6 lg:px-10">
        <Link
          ...
          className={cn(
            ...
            onGradient ? "text-white" : "text-slate-900",
          )}
        >
          AgencyFlow<span className="text-[var(--landing-green)]">.</span>
        </Link>
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-3 px-5 py-3 text-xs font-bold uppercase tracking-[0.2em] shadow-sm transition",
            onGradient
        ...
        <div className="mt-auto space-y-5 px-8 py-8">
          <div className="flex gap-3">
            {socials.map((label) => (
              <span
                key={label}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500 transition hover:bg-indigo-600 hover:text-white"
              >
                {label}
              </span>
            ))}
